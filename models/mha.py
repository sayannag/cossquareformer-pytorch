...
class MHA(nn.Module):
    ...
    def __init__(self, d_model, n_heads, use_cos, kernel, dropout, denom_eps, bias):
        ...
        if use_cos:
            self.attention_func = self.cos_square_linear_attention
        else:
    ...
    def cos_square_linear_attention(self, q, k, v, weights):
        """Implements linear attention with cos-square reweighting as in'Deciphering Environmental Air Pollution with Large Scale City Data' (https://arxiv.org/pdf/2109.04572.pdf).
        ...
        output = torch.einsum('bsnz,bsn->bsnz', qkv + qkv_cos +
